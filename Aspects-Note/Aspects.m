...
/**
 __unused 用于修饰函数与变量，表示这个变量或者函数不使用
 */
...
/**
 第一步创建AspectContainer 并且通过objc_getAssociatedObject绑定到hook的类中
 1、里面包含的信息有 before、instead、after的三个数组
    aspect_getContainerForObject(self, selector);
 2、创建AspectIdentifier 实例，根据option(after、instead、before) 对应到container中的三个数组中
    [AspectIdentifier identifierWithSelector:selector object:self options:options block:block error:error]
 3、aspect_prepareClassAndHookSelector
 
 ----+(id<AspectToken>)aspect_hookSelector: withOptions: usingBlock: error:
    |____aspect_add
 */
...
/**
 传入一个selector 返回带有aspecct前缀的selector
 */
...
/**
 比对方法签名与传入对象 的selector生成的方法签名是否一致
 
 @param blockSignature 已有的方法签名
 @param object 传入的对象
 @param selector 传入的方法
 @param error error
 @return 返回是否一致
 */
...
/**
 1、根据selector 和 class 找到Method
    class_getInstanceMethod(klass, selector);
 2、再根据Method 找到IMP
    method_getImplementation(targetMethod);
 3、判断IMP是否是已经转发过了的
 4、就是MethodSwizzle 的两个方法了,这里都给原始的selector 添加了前缀
    没有就添加 class_addMethod
    有了点话就替换 class_replaceMethod
...
static Class aspect_hookClass(NSObject *self, NSError **error) {
    NSCParameterAssert(self);
	Class statedClass = self.class; //self 为实例是 返回的是类对象，类方法中的self传入 返回的也是类对象
	Class baseClass = object_getClass(self); //self为一个类对象返回的就是isa指向的metaclass
	NSString *className = NSStringFromClass(baseClass);
    NSLog(@"state==%@ base===%@",statedClass, baseClass);
    // Already subclassed
	if ([className hasSuffix:AspectsSubclassSuffix]) {
		return baseClass;
	...
	if (subclass == nil) {
		subclass = objc_allocateClassPair(baseClass, subclassName, 0); //动态生成一个子类
		if (subclass == nil) {
		...
		aspect_hookedGetClass(subclass, statedClass); //将动态生成的子类的 [subclass class]方法hook掉，使其返回得到父类的类名
		aspect_hookedGetClass(object_getClass(subclass), statedClass); //子类指向的metaclass 也做同样的处理
		objc_registerClassPair(subclass); //注册子类
	}
...
/**
 swizzle forwardInvocation方法，
 class_replaceMethod() 传入要修改的类，要修改的方法名，替代原来IMP的IMP，typeEncoding，返回的是原来的IMP
...
/**
 ...
 class_replaceMethod(class, selector, imp, typeEncoding)
 */


/**
 将已经swizzle过 forwardInvocation 的类还原的具体实现
...
/**
 修改一个类的 [obj class]的返回值
 */
static void aspect_hookedGetClass(Class class, Class statedClass) {
    NSCParameterAssert(class);
    NSCParameterAssert(statedClass);
    NSLog(@"%s--%@---%@",__func__, class, statedClass);
...
/**
 这里就是一个通用的block 同时生成全局的集合，用来放已经swizzle过的类名
 */
...
/**
 将已经swizzled的类放到一个全局的集合中
 这里会调用aspect_swizzleForwardInvocation()
 */
...
/**
 这里是hook掉的方法被调用的地方,比如说hook了 viewDidLoad,添加的方法肯定是aspects__viewDidLoad，IMP交替时候调用viewDidLoad其实是在调用aspects__viewDidLoad

 @param self 调用类
 @param selector 这里的selector 是forwardInvocation:
...
/**
 获取全局的一个mutableDic 来存放已经hook的类，对应的key-value 为【hookClass ： AspectTracker】
...
/**
 这个函数的逻辑就是判断一个selector 是否是黑名单比如说像retain、release、autorelease、forwardInvocation
 
 还有特殊情况:
 1、dealloc hook的时机不为before 肯定不可行
 2、hook的类 和 实例都不响应该selector
 3、看hook类的父类们是否被hook了
 */
...
/**
 获取NSInvocation 中方法签名（meghodSignature） 的参数
