...
class Medium_018_4Sum: NSObject {
    class Pair {
            ...
            if first + last > target {
                ...
                if let unwrappedFirst: [Pair] = map[first] {
                    for a: Pair in unwrappedFirst {
                        if a == lastA { // Cannot convert value of type 'Medium_018_4Sum.Pair' to expected argument type '_OptionalNilComparisonType'
                            continue
                        }
                        lastA = a
                        var lastB: Pair? = nil
