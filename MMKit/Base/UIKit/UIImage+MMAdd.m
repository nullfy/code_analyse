...
MMSYNTH_DUMMY_CLASS(UIImage_MMAdd)

static NSTimeInterval _mm_CGImageSourcesGetGIFFrameDelayAtIndex(CGImageSourceRef source, size_t index) {
...
+ (UIImage *)imageWithSmallGIFData:(NSData *)data scale:(CGFloat)scale {
    ...
    for (size_t i = 0; i < count; i++) {
        NSTimeInterval delay = _mm_CGImageSourcesGetGIFFrameDelayAtIndex(source, i);
        totalTime += delay;
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        CGContextRef context = CGBitmapContextCreate(NULL, width, height, 0, 0, space, bitmapInfo);
...
+ (UIImage *)imageWithPDF:(id)dataOrPath {
    return [self _mm_imageWithPDF:dataOrPath resize:NO size:CGSizeZero];
}

+ (UIImage *)imageWithPDF:(id)dataOrPath size:(CGSize)size {
    return [self _mm_imageWithPDF:dataOrPath resize:YES size:size];
}

+ (UIImage *)_mm_imageWithPDF:(id)dataOrPath resize:(BOOL)resize size:(CGSize)size {
    CGPDFDocumentRef pdf = NULL;
    if ([dataOrPath isKindOfClass:[NSData class]]) {
        CGDataProviderRef provide = CGDataProviderCreateWithCFData((__bridge CFDataRef)dataOrPath);
        pdf = CGPDFDocumentCreateWithProvider(provide);
    ...
    CGContextRef context = CGBitmapContextCreate(NULL, pdfSize.width * scale, pdfSize.height * scale, 8, 0, colorSpace, kCGBitmapByteOrderDefault | kCGImageAlphaPremultipliedFirst);
    if (!context) {
...
+ (UIImage *)imageWithEmoji:(NSString *)emoji size:(CGFloat)size {
    ...
    NSAttributedString *string = [[NSMutableAttributedString alloc] initWithString:emoji
    ...
    CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
    CGContextRef context = CGBitmapContextCreate(NULL, size * scale, size * scale, 8, 0, colorSpace, kCGBitmapByteOrderDefault | kCGImageAlphaPremultipliedFirst);
...
- (void)drawInRect:(CGRect)rect withContentMode:(UIViewContentMode)contentMode clipsToBounds:(BOOL)clips {
    CGRect drawRect = MMCGRectFitWithContentMode(rect, self.size, contentMode);
...
- (UIImage *)_mm_flipHorizontal:(BOOL)horizontal vertical:(BOOL)vertical {
    ...
    if (horizontal) vImageHorizontalReflect_ARGB8888(&src, &dest, kvImageBackgroundColorFill);
...
- (UIImage *)imageByRotateLeft90 {
    return [self imageByRotate:MMDegreesToRadians(90) fitSize:YES];
}
...
- (UIImage *)imageByRotate180 {
    return [self _mm_flipHorizontal:YES vertical:YES];
}

- (UIImage *)imageByFlipVertical {
    return [self _mm_flipHorizontal:NO vertical:YES];
}

- (UIImage *)imageByFlipHorizontal {
    return [self _mm_flipHorizontal:YES vertical:NO];
}
...
- (UIImage *)_mm_mergeImageRef:(CGImageRef)effectCGImage
                 ...
                 tintBlendMode:(CGBlendMode)tintBlendMode
                     maskImage:(UIImage *)maskImage opaque:(BOOL)opaque {
    ...
    if (!hasTint && !hasMask) return [UIImage imageWithCGImage:effectCGImage];
    
    UIGraphicsBeginImageContextWithOptions(size, opaque, scale);
    CGContextRef context = UIGraphicsGetCurrentContext();
    CGContextScaleCTM(context, 0, -size.height);
    if (hasMask) {
        CGContextDrawImage(context, rect, self.CGImage);
        CGContextSaveGState(context);
    ...
    if (hasMask) CGContextRestoreGState(context);
