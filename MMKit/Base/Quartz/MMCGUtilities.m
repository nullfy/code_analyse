...
CGContextRef MMCGContextCreateARGBBitmapContext(CGSize size, BOOL opaque, CGFloat scale) {
...
CGContextRef MMCGContextCreateCrayBitmapContext(CGSize size, CGFloat scale) {
    size_t width = ceil(size.width * scale);
    size_t heidht = ceil(size.height * scale);
    if (width < 1 || heidht < 1) return NULL;
    
    CGColorSpaceRef space = CGColorSpaceCreateDeviceGray();
    CGImageAlphaInfo alphaInfo = kCGImageAlphaNone;
    CGContextRef context = CGBitmapContextCreate(NULL, width, heidht, 8, 0, space, kCGBitmapByteOrderDefault | alphaInfo);
    CGColorSpaceRelease(space);
    if (context) {
        CGContextTranslateCTM(context, 0, heidht);
        CGContextScaleCTM(context, scale, -scale);
    }
    return context;
}
...
CGAffineTransform MMCGAffineTransformGetFromViews(UIView *from, UIView *to) {
    ...
    return MMCGAffineTransformGetFromPoints(before, after);
}
