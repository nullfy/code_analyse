...
MMSYNTH_DUMMY_CLASS(NSObject_MMAdd)

@implementation NSObject (MMAdd)

/*
 在iOS中可以直接给某个对象发消息的方式有两种
 一种是performSelector:withObjcect;    对大于两个的参数或者有返回值的处理不行
 再一种就是NSInvocation                 可以处理多个参数和有返回值的消息
 这里的define中\ 是用来换行的
 
 1、创建一个函数签名，这个签名可以是任意的，但需要注意的是 签名函数的参数数量要和调用的一致
...
- (id)performSelectorWithArgs:(SEL)sel, ...{
    INIT_INV(sel, nil);     //这里需要调用setInv: withSig: andArgs 方法
    [inv invoke];
    return [NSObject getReturnFromInv:inv withSig:sig];
}

- (void)performSelectorWithArgs:(SEL)sel afterDelay:(NSTimeInterval)delay, ... {
    INIT_INV(delay, );  //多个参数写到最末位一个  没有返回值第二个就不写
    [inv retainArguments];
...
- (id)performSelectorWithArgs:(SEL)sel onThread:(NSThread *)thread waitUntilDone:(BOOL)wait, ... {
    INIT_INV(wait, nil);
    if (!wait) [inv retainArguments];
    [inv performSelectorWithArgs:@selector(invoke) onThread:thread waitUntilDone:wait];
    return wait ? [NSObject getReturnFromInv:inv withSig:sig] : nil;
}
...
#pragma mark    下面的set 和 get 方法为上面的宏 所调用    INIT_INV()
...
return @(ret); \
    ...
    switch (*type) {
        ...
        case 'd': return_with_number(double); //没有 unsigned float
        case 'D': {
            long double ret;
            [inv getReturnValue:&ret];
