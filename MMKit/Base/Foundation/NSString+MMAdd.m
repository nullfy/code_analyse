...
- (NSString *)stringByURLEncode {
    if ([self respondsToSelector:@selector(stringByAddingPercentEncodingWithAllowedCharacters:)]) {
        ...
        NSMutableCharacterSet *allowedCharacterSet = [[NSMutableCharacterSet URLQueryAllowedCharacterSet] mutableCopy];
...
#pragma clang   diagnostic ignord "-Wdeprecated-declarations"
...
#pragma clang   diagnostic ignored "-Wdeprecated-declarations"
        ...
        NSString *decoded = [self stringByReplacingOccurrencesOfString:@"+"
                                                            withString:@" "];
        decoded = (__bridge_transfer NSString *)CFURLCreateStringByReplacingPercentEscapesUsingEncoding(NULL,
...
- (NSString *)stringByReplaceingRegex:(NSString *)regex options:(NSRegularExpressionOptions)options withString:(NSString *)replacement {
    NSRegularExpression *pattern = [NSRegularExpression regularExpressionWithPattern:regex options:options error:nil];
    if (!pattern) return  self;
    return [pattern stringByReplacingMatchesInString:self options:NSMatchingReportProgress range:NSMakeRange(0, self.length) withTemplate:replacement];
}
...
- (void)enumerateUTF32CharInRange:(NSRange)range usingBlock:(void (^)(UTF32Char char32, NSRange range, BOOL *stop))block {
    ...
    BOOL stop = NO;
    NSRange subRange;
    UTF32Char oneChar;  //这里为什么可以用UTF32Char ＊ 也可以不用指针
...
- (CGFloat)pathScale {
    ...
    [name enumerateRegexMatches:@"@[0-9]+\\.?[0-9]*x$" options:NSRegularExpressionAnchorsMatchLines usingBlock:^(NSString * _Nonnull match, NSRange matchRange, BOOL stop) {
