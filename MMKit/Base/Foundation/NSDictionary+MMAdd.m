...
#import "NSDictionary+MMAdd.h"
#import "NSString+MMAdd.h"
#import "NSData+MMAdd.h"
#import "MMKitMacro.h"

MMSYNTH_DUMMY_CLASS(NSDictionary_MMAdd)
@interface __MMXMLDictionaryParser : NSObject <NSXMLParserDelegate>

@end

@implementation __MMXMLDictionaryParser {
...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary<NSString *,NSString *> *)attributeDict {
    ...
    if (!_root) node[XMLText] = elementName;
    ...
    if (_root) {
        ...
        _root = node;
        _stack = [NSMutableArray arrayWithObjects:node, nil];
    }
}
...
- (void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName {
    ...
    if (!nodeName) {
        for (NSString *name in topNew) {
            ...
            if (obj == topNew) {
                nodeName = name; break;
            } else if ([obj isKindOfClass:[NSArray class]] && [obj containsObject:top]) {
                nodeName = name; break;
            }
        }
    }
    ...
    if ([parent isKindOfClass:[NSArray class]]) {
        NSArray *parentArray = parent;
        parent[parentArray.count - 1] = inner;
    } else {
...
@implementation NSDictionary (MMAdd)

+ (NSDictionary *)dictionaryWithPlistData:(NSData *)plist {
    if (!plist) return nil;
    NSDictionary *dic = [NSPropertyListSerialization propertyListWithData:plist options:NSPropertyListImmutable format:NULL error:NULL];
...
- (NSArray *)allValuesSortedByKeys {
    NSArray *sortKeys = [self allKeysSorted];
...
+ (NSDictionary *)dictionaryWithXML:(id)xmlDataOrString {
    __MMXMLDictionaryParser *parser = nil;
    if ([xmlDataOrString isKindOfClass:[NSString class]]) {
        parser = [[__MMXMLDictionaryParser alloc] initWithString:xmlDataOrString];
    } else if ([xmlDataOrString isKindOfClass:[NSData class]]) {
        parser = [[__MMXMLDictionaryParser alloc] initWithData:xmlDataOrString];
    }
...
@implementation NSMutableDictionary (MMAdd)

+ (NSMutableDictionary *)dictionaryWithPlistData:(NSData *)plist {
    if (!plist) return nil;
    NSMutableDictionary *dic = [NSPropertyListSerialization propertyListWithData:plist options:NSPropertyListMutableContainersAndLeaves format:NULL error:NULL];
    if ([dic isKindOfClass:[NSMutableDictionary class]]) return dic;
    return nil;
}
